...
const OURS: CompactOptions = {
  ...
  alwaysKeepResult: [
    '^(Agent|TaskOutput|WebFetch|WebSearch)$',
...
if (!path) {
  console.error('usage: tsx eval/run.ts <session.jsonl> [upstream|ours|ours-call04]');
  process.exit(2);
}
const messages = transcriptFromJsonl(readFileSync(path, 'utf8'));
console.log(
  `transcript: ${messages.length} messages, ${messages.reduce((n, m) => n + m.toolUses.length, 0)} tool uses`,
);
...
for (const [name, options] of Object.entries(CONFIGS)) {
  ...
  try {
    ...
    for (const d of result.decisions) {
      ...
      console.log(
        `${d.id} ${d.tool} ${d.action}${d.inputTruncated ? '+input' : ''} call=${d.keepCall.toFixed(2)} result=${d.keepResult.toFixed(2)}${input}${flag} ${head}`,
      );
    }
  } catch (error) {
    console.log(
